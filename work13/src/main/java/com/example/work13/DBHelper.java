...
public class DBHelper extends SQLiteOpenHelper {
    //    有没有.db都可以
    public static String DB_NAME = "work13.db";
...
//    }
    ...
    public void onCreate(SQLiteDatabase db) {
        String sql = "create Table TbWork(_id integer not null primary key autoincrement," +//id integer，不为空，主键，自增长，默认自增
                "data varchar(100) not null unique)";
        db.execSQL(sql);//执行SQL语句
//        添加的两种方式
//        sql="insert into Muser values(null,'admin','admin','无形装X_最为致命')";
        sql = "insert into TbWork values(?,?)";
        db.execSQL(sql, new Object[]{null, "admin"});
    }
